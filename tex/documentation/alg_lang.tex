...
ALG Library is a collection of standard ALG functions that can be safely \lfunc{CALL}ed.
Moreover, in function \lfunc{ESTIMATE}, predefined compressed sensing algorithms are called which are also in the ALG library.
...
\subsubsection{OMP}\index{OMP}
Orthogonal matching pursuit\index{orthogonal matching pursuit}. (\texttt{OMP.alg})

\subsubsection{OMPL}\index{OMPL}
OMP List\index{OMP list}\index{orthogonal matching pursuit list}. (\texttt{OMPL.alg})
...
\subsubsection{Oracle LS}\index{Oracle LS}
Oracle least square (LS). (\texttt{Oracle\_LS.alg})

\subsubsection{LS Support}\index{LS support}
Least square (LS) with specified support. (\texttt{LS\_Support.alg})

\subsection{Other Utilities}

\subsubsection{max\_n}\index{max\_n}
